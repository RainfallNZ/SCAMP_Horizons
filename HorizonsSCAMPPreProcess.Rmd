...
SubZoneLanduseShapeFile         <- file.path(GISDataDirectory,"ZoneLanduse","ZoneLanduse")                   #May be created if necessary in QGIS by interesectiong LandUse and Subzone spatial data
SubZoneLanduseAggShapeFile         <- file.path(GISDataDirectory,"ZoneLanduse","ZoneLanduseAgg")
...
LeachRateLUTFile                   <- file.path(DataDirectory,"LandUseToLossRatesV0.csv")
...
ExtraRECReachLabelsFile             <- file.path(DataDirectory,"ManualRiverNameLookUpTableV0.csv")

#Set some excel spreadsheet names
PointSourceExtraDataFile           <- file.path(DataDirectory,"pointSourcesTN_2012And2017.xlsx")
PointSourceYear                    <- "2017" #2012 is the default and has been used for everything except for one scenario, The other option is "2017"
...
#Load the Horizon total nitrogen load measurement sites, previously prepared by Caroline Fraser and called "HZLoads
...
#Load point source data from a spreadsheet that Ton prepared. This has 2012 and 2017 data. 2017 data are the default data used.
...
# #For mapping purposes it is useful to know which SCAMP model each zone is within. This is (mostly) done by the following few lines. However, for zones bordering two models, it sometimes selects the wrong model domain, so this file has been externally manually edited. Be sure that you want to overwrite it!
...
PointSourceTable$"Annual TN Load (kg/yr)" <- round(PointSourceSites$TN.kgpy[match(PointSourceTable$nzsegment,PointSourceSites$nzsegment)],0)
PointSourceTable$"Annual TP Load (kg/yr)" <- round(NA,0)
...
MeasurementSiteTable$'TN Load (kg/yr)' <- round(MeasurementSites$load[match(MeasurementSiteTable$"Site Name or No", MeasurementSites$sID)],1)
MeasurementSiteTable$'TN Load Lci (kg/yr)' <- round(MeasurementSites$load_Lci[match(MeasurementSiteTable$"Site Name or No", MeasurementSites$sID)],1)
MeasurementSiteTable$'TN Load Uci (kg/yr)' <- round(MeasurementSites$load_Uci[match(MeasurementSiteTable$"Site Name or No", MeasurementSites$sID)],1)

MeasurementSiteTable$'TP Load (kg/yr)' <- round(NA,1)
MeasurementSiteTable$'TP Load Lci (kg/yr)' <- round(NA,1)
MeasurementSiteTable$'TP Load Uci (kg/yr)' <- round(NA,1)
```
...
#Now create a bigger version, with a row for each of the different landuse within each sub-management zone
...
saveWorkbook(Out, file.path(DataDirectory,"SCAMP-Inputs_HorizonsV1Test.xlsx"), overwrite = T)
```
